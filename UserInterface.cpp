...
int UserInterface::run(){
  ...
  renderWindow_->SetSize(1280, 680);
  ...
  geometryFilter_->SetInputData(editor_->getData());
  geometryFilter_->Update();
  
  vtkPolyData *surface = geometryFilter_->GetOutput();

  boundaryMapper_->SetInputData(surface);
  boundaryMapper_->SetScalarRange(surface->GetScalarRange());
  
  mainActor_->SetMapper(boundaryMapper_);
  
  mainActor_->GetProperty()->SetOpacity(0.3);
  renderer_->AddActor(mainActor_);
